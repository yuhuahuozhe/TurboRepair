...
class AdminHomeService extends BaseProjectAdminService {
	...
	async adminHome() {
		...
		let waitTaskCnt = await TaskModel.count({ TASK_STATUS: TaskModel.STATUS.WAIT });
		let apptTaskCnt = await TaskModel.count({ TASK_STATUS: TaskModel.STATUS.APPT });
		let runTaskCnt = await TaskModel.count({ TASK_STATUS: TaskModel.STATUS.RUN });
		let overTaskCnt = await TaskModel.count({ TASK_STATUS: TaskModel.STATUS.OVER });
		return { waitTaskCnt, apptTaskCnt, runTaskCnt, overTaskCnt };

...
class AdminTaskController extends BaseProjectAdminController {
	...
	async apptTaskMember() {
		await this.isAdmin();

		// 数据校验
		let rules = {
			taskId: 'must|id',
			memberId: 'must|memberId',
		};
		...
		return await service.apptTaskMember(this._admin, input.taskId, input.memberId);
	}
	...
	async statusAdminTask() {
		...
		let service = new AdminTaskService();
		return await service.statusAdminTask(input.id, input.status);
	}
